...
{
    public class beanweapon : beanaimanager
    {
        ...
        protected virtual void pool()
        {
            ...
            objectpooler.createenemypoolbean(weapon, currentpool, projectileparentfolder, this);
        ...
        {
            if (onlyfirewhenclose && beanenemy.playerisclose || !onlyfirewhenclose)
            {
        ...
        {
            if (autofire && onlyfirewhenclose && beanenemy.playerisclose || autofire && !onlyfirewhenclose)
            {
        ...
        protected virtual void aim()
        {
            if (player != null && aimatplayer)
            {
                ...
                if (!beanenemy.facingleft)
                {
                    target.x = target.x - currentposition.x;
                    target.y = player.GetComponent<Collider2D>().bounds.center.y - currentposition.y;
                }
                else
                {
                    target.x = currentposition.x - target.x;
                    target.y = currentposition.y - player.GetComponent<Collider2D>().bounds.center.y;
                }
